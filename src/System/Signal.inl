...
template <typename... Args>
Signal<Args...>::Signal(Signal<Args...>&& other)
...
SignalConnection& Signal<Args...>::Connect(
    InstanceType& instance, void (MFInstanceType::*func)(Args...)) {
    static_assert(!std::is_pointer<InstanceType>::value,
                  "Instance cannot be a pointer, it must be a reference");
    static_assert(std::is_member_function_pointer<decltype(func)>::value,
                  "Function must be a non-static member function pointer");
    static_assert(std::is_base_of<MFInstanceType, InstanceType>::value,
                  "Instance is not base of member function");
    return Connect([&instance, func](Args... args) {
...
template <typename... Args>
template <typename InstanceType, typename MFInstanceType>
SignalConnection& Signal<Args...>::Connect(InstanceType& instance,
                                           void (MFInstanceType::*func)(Args...)
                                               const) {
    static_assert(!std::is_pointer<InstanceType>::value,
                  "Instance cannot be a pointer, it must be a reference");
    static_assert(std::is_member_function_pointer<decltype(func)>::value,
...
template <typename... Args>
SignalConnection& Signal<Args...>::Connect(
    const typename Signal<Args...>::SlotType& slot) {
    std::lock_guard<std::mutex> lock(m_slots_mutex);
    SignalConnection::IdType id = ++m_current_id;
    SignalConnection connection(id, [this](SignalConnection& connection) {
        this->Disconnect(connection);
    });
    auto pair =
        m_slots.emplace(id, std::make_pair(std::move(connection), slot));
