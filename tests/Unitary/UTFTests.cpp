...
TEST_CASE("Calling utf::GetCodePoint", "[UTF]") {
    std::array<uint8, 4> smile8 = {0xF0, 0x9F, 0x98, 0x80};  // 😀
    std::array<uint16, 2> smile16 = {0xD83D, 0xDE00};        // 😀
    std::array<uint8, 4> earth8 = {0xE5, 0x9C, 0xB0, 0x00};  // 地
    std::array<uint16, 2> earth16 = {0x5730, 0x0000};        // 地
    ...
    SECTION("Should return the next code unit") {
        {
            ...
            auto it3 = utf::Next<utf::UTF_8>(it2, end);

            REQUIRE(&(*it0) == &(*it1));
            REQUIRE(&(*(begin + 4)) == &(*it1));
            REQUIRE(&(*(begin + 8)) == &(*it2));
            REQUIRE(&(*(begin + 11)) == &(*it3));
        }
        {
            std::basic_string<char16> smiley = u"\U0001F600\U0001F603\U00005730";  // "😀😃地"
            ...
            auto it3 = utf::Next<utf::UTF_16>(it2, end);
        ...
        {
            std::basic_string<char32> smiley = U"\U0001F600\U0001F603\U00005730";  // "😀😃地"
            ...
            auto it3 = utf::Next<utf::UTF_32>(it2, end);
