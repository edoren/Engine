...
std::vector<float> CreateNoiseMap(const math::ivec2& size, const math::vec2& offset, float scale,
...
int main(int argc, char* argv[]) {
    ...
    std::vector<float> noise_map = CreateNoiseMap(image_size, offset, scale, noise);
    std::vector<Color32> color_map = CreateColorMap(image_size, noise_map, terrains);
    ...
    while (!input.exit_requested()) {
        ...
        if (update_map) {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, image_size.x, image_size.y, GL_RGBA, GL_UNSIGNED_BYTE, color_map.data());
        ...
        render.ClearFrameBuffer(math::vec4(0, 0, 0, 0));
